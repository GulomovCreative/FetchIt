...
- **FormIt 5.2 и новее** умеет отправлять формы через AJAX сам. Для форм FetchIt этот режим выключается: FetchIt убирает сохранённые FormIt параметры, плейсхолдер `fi.ajaxToken` и скрипт `formit.js`, иначе форму можно было бы отправить через `action.php` FormIt в обход защиты. reCAPTCHA из FormIt 5.2 без `formit.js` не работает, поэтому для форм FetchIt включайте капчу через `fetchit.captcha`.
