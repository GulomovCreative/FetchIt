...
Стили подключаются первыми в `<head>`, с селекторами из одного класса. Они сильнее общих правил сайта для элементов (`button { … }`) и проигрывают любому правилу сайта с классом. Правила внутри `@layer` (например, в Tailwind 4) им проигрывают, для них проще менять переменные. По умолчанию это цвета Tailwind CSS 4, у которых белый текст читается с достаточным контрастом (WCAG AA): `green-700` для успеха и `red-600` для ошибки. Там, где браузер понимает `oklch()`, они заданы точно, в остальных браузерах в hex:

```css
.fetchit-toasts {
  --fetchit-toast-color: #fff;
  --fetchit-toast-success: oklch(52.7% 0.154 150.069); /* green-700, #008236 */
  --fetchit-toast-error: oklch(57.7% 0.245 27.325);    /* red-600, #e7000b */
}
```

На сайте с Tailwind 4 можно взять его переменные: `--fetchit-toast-success: var(--color-emerald-700)`.
